...
/** Runs the agent for the loan's next period and waits for it to finish. */
export async function runAgentForLoan(
  loanId: string,
  startedByUid: string | null = null,
) {
  const { execute } = await startAgentRun(loanId, startedByUid);
  return execute();
}

/**
 * Validates the loan and creates the agentRuns doc (status "running"), so
 * dashboards see the run immediately. `execute()` then runs verify →
 * reconcile → decide → explain, anchors the result on-chain and mirrors it
 * into Firestore. The loan doc only advances if everything succeeds, so a
...
export async function startAgentRun(
  ...
  // Denormalized onto every doc this run writes (see firestore.rules).
  const scope = { ownerUid: loan.ownerUid, rmUid: loan.rmUid };
  ...
  // Borrower-readable marker so every persona sees "assessment in progress"
  // (agentRuns itself is staff-only). Cleared on completion or failure.
  await loanRef.update({
    activeRun: { runId, period, startedAt: FieldValue.serverTimestamp() },
  });

  return { runId, execute: executeRun };
  ...
  async function executeRun() {
    ...
    try {
      ...
      await runRef
        ...
        .catch(() => undefined);
      await loanRef
        .update({ activeRun: FieldValue.delete() })
        .catch(() => undefined);
