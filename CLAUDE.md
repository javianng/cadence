...
- `loans/{loanId}` — terms (`baseMarginBps`, `floorBps`, `capBps`, `maxStepBps`, `bands`, `staticMarginBps`), `ownerUid`, `rmUid`, `scenario`, `tokenId`, `mintTxHash`, `contractAddress`, `chainId`; live state `currentMarginBps`/`pendingAdjustmentBps` (read from the contract), `currentScore`, `scoreDelta`, `currentPeriod`, `status` (`on_track`|`watch`|`under_review`), `activeRun` (set while an agent run is in flight; borrower-readable).
...
- `exceptions/{loanId}__pNN` (codes, status `open`|`info_requested`|`resolved`|`superseded`, rmBrief, recommendation, decision, decisionTxHash, agreedWithAgent), `events/{loanId}__pNN__{type}` with `visibility` (`all` = borrower-visible, `internal` = staff only), `messages` (borrower ↔ RM), `simulation/state`. `agentRuns` is written by the agent.
- **Every loan-scoped doc carries `ownerUid` + `rmUid`** (the seed backfills them); `firestore.rules` scopes reads by role: borrower → own loans and `visibility == 'all'` events only (never `exceptions`/`agentRuns`); RM → own book (`rmUid`); risk → everything. Client queries must include the matching `where` (see `src/lib/data/hooks.ts`). Publish rules with `npm run rules:deploy`.
...
**Dashboards (built)** — shared pieces in `src/components/cadence/` (charts, Loan360, ExceptionReview, sections); live data via `src/lib/data/hooks.ts`; pure metrics in `src/lib/data/metrics.ts` (tested).
- Borrower (`/borrower`, `/kpis`, `/pricing`, `/what-if`): look and ask only — facility switcher (one borrower owns all 3 demo loans), neutral "under review" notice, message RM, client-side what-if using `scoring.ts` + `pricing.ts`.
- RM (`/rm`, `/rm/exceptions`, `/rm/exceptions/[id]`, `/rm/loans/[loanId]`): severity-sorted queue (rule weights STEP_UP/KPI_BREACH 3, DATA_MISMATCH/ANOMALY/SHARP_DECLINE 2, DATA_GAP 1, +2 pending step-up); decisions via `POST /api/exceptions/[id]/decision` — every action is on-chain: approve → `resolveAdjustment(true)`, hold → `resolveAdjustment(false)` or `flagException(RM_HOLD)`, request info → `flagException(RM_REQUEST_INFO)`.
- Risk (`/risk`, `/risk/governance`, `/risk/loans/[loanId]`, `/risk/exceptions/[id]`): read-only oversight, KPI drift heatmap, RM–agent agreement/override rate, decision log + CSV export.
- `/demo`: presenter page; `POST /api/agent/run` is async (202 + `runId`, `?wait=1` to block) and requires a signed-in user. All API routes take `Authorization: Bearer <Firebase ID token>` (`requireUser` in `src/lib/server/auth.ts`; client helper `apiPost`).
- Charts use the dataviz reference palette (tokens `--series-1..3`, `--status-*`, `--div-*` in `globals.css`); loan colour is fixed by its slot in `DEMO_LOANS`.
